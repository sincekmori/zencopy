...
The default action is **Zen**: it distills the copied text to its essence in one short sentence, in your chosen language. Actions are configurable, and you can add your own.
...
- **Routing you control** — which action runs is decided by declarative rules (capture kind, plus app / window / URL / length overrides), edited in the GUI or as a file.
...
- **GUI (Settings window)** — AI provider (a free Gemini key is the fastest start), the actions editor, routing (per-kind assignments and override rules), popup position, theme, language, launch at login. Stored per-user.
