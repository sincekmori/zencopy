...
## Один провайдер, одна модель {#one-provider-one-model}
...
## Локальная Ollama — ничего не покидает компьютер {#local-ollama--nothing-leaves-your-machine}
...
## Две роли: быстрая по умолчанию, умная по запросу {#two-roles-fast-by-default-smart-on-demand}
