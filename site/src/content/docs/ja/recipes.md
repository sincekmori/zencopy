...
ファイルは[アプリ設定ディレクトリ](/ja/configuration/#設定ファイル上級者向け)に置きます。編集は次のトリガーから反映されます。
...
## role を 2 つ: 普段は速く、要所は賢く

role はアクションとモデルを疎結合にします。
アクションは role 名だけを持ち（フロントマターに `role: smart`）、それがどのモデルを意味するかはここで決めます。アクションに触れずにいつでも差し替えられます。
