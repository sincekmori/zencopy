...
## Ollama محلی — هیچ چیزی از دستگاه شما خارج نمی‌شود
...
## دو نقش: پیش‌فرضِ سریع، هوشمند در صورت نیاز

نقش‌ها اکشن‌ها را از مدل‌ها جدا می‌کنند.
اکشن‌ها یک نقش را نام می‌برند (`role: smart` در frontmatter اکشن)؛ این‌که آن نقش یعنی کدام مدل، همین‌جا تعیین می‌شود — هر وقت بخواهید بدون دست زدن به اکشن‌ها عوضش کنید.
