...
- A ★ marca a ação padrão — a que um copiar duplo executa primeiro.
...
Condições de `when`: `kind`, `app_name`, `exec_name`, `window_title`, `url` (todas aceitam o curinga `*`), mais `min_chars` / `max_chars`.
