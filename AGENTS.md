...
A demo whose lines name the chord comes as three cuts, `<demo>.mp4` saying Ctrl/⌘ + C + C (for a visitor whose OS the page cannot tell, as with the site's OS wording), `<demo>.ctrl.mp4` saying Ctrl + C + C and `<demo>.cmd.mp4` saying ⌘ + C + C (one poster, the default's), and the docs head script swaps the source by OS; a locale without an entry gets uncaptioned videos, so captions are part of translating a locale, followed by regenerating it.
