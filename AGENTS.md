...
A demo can carry captions: the page's MDX passes a `<Fragment slot="cues">` holding one `<p>` per beat, in order (the first demo has four — the mail as it is, the selection, the chord, the summary complete), and [DemoVideo.astro](site/src/components/DemoVideo.astro) shows them over the video one at a time from the moments the generator wrote for that locale into [scripts/demo-video/cues.json](scripts/demo-video/cues.json); the OS components (`<CopyTwice />`, `<OsText />`) work inside them.
The moments come from the page beats and the session's own clock, so a locale's captions need its videos generated with the current beats — a page that captions a demo the file has no moments for fails the site build, saying which locale to regenerate.
