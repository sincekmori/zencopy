...
Every scenario is rendered three times — for a visitor of unknown OS (the key chord spelled `Ctrl/⌘`, the neutral form the site's animation and OS wording fall back to), as Windows and as macOS (the app spells the chord off the user agent): a scenario flagged `os` in the registry lands as `<scenario>.png` (neutral), `<scenario>.ctrl.png` and `<scenario>.cmd.png`, and the docs embed it through [site/src/components/Screenshot.astro](site/src/components/Screenshot.astro), which the head script swaps to the Ctrl or ⌘ file for a visitor whose OS it knows; a scenario without the flag must render identically in every locale, or the runner fails naming the shot to flag (a flagged scenario's renders may coincide in a locale where a dialog covers the chord — it still lands three times, so the docs' references hold).
...
Two files in every locale's `screenshots/` folder are not renders but hand-taken captures of Windows itself: `win-smartscreen-more-info.png` and `win-smartscreen-run-anyway.png`, the SmartScreen screens an unsigned installer meets, shot as 534×500 window captures under each of the 19 display languages (fa, id, vi included — their packs do translate this dialog). Microsoft permits screenshots of its products in documentation on the condition that they are unaltered (resizing is the only edit allowed; no cropping, no annotation) and that the use is credited with "Used with permission from Microsoft." — the docs embed them through [site/src/components/MicrosoftScreenshot.astro](site/src/components/MicrosoftScreenshot.astro), a figure whose caption carries that sentence under each capture, so a translation keeps the component and the credit comes with it; put any "press here" into the prose. The second capture shows the installer's file name with the version it was shot with (0.16.0); the text beside it says the number may be older, so the captures need no retake per release. Screens that exist in one language only live in `site/public/screenshots/` (no locale segment) and every locale's page references them there: `win-installer-welcome.png`, `-location.png`, `-complete.png` and `-finish.png` are the NSIS installer's four pages (the installer ships in English alone), 499×388 window captures, the last one with the desktop-shortcut box cleared — the state the installer starts in since the hook under Constraints.
...
A demo can carry captions, burned into the video: [scripts/demo-video/captions.ts](scripts/demo-video/captions.ts) holds one line per beat, per locale and demo (the first demo's four — the mail as it is, the selection, the chord, the summary complete), with `{lang}` / `{lang:in}` / `{lang:into}` for the viewer's language in the form the sentence needs (from [src/lib/language-forms.ts](src/lib/language-forms.ts)); WebKit sets each line in the site's font stack and ffmpeg lays it over the video from its beat's moment, which the generator takes off the page beats and the session's own clock, never off the footage.
The chord is shown, not spelled: its line, the third, writes `{keys}` where the keys go, with the locale's own words around them (`{keys} を押すと`) — the compiler holds every locale to the four beats and to the token — and the generator draws the hero animation's keycaps there, in the line — the modifier pressed and held, C tapped twice, both released as the popup lands — from the very stylesheet the landing page and the welcome screen animate with ([src/assets/hero-demo.css](src/assets/hero-demo.css)): WebKit holds the animation still and is stepped through it a video frame at a time, so the motion and its timing come from that one place and the keys themselves need no translating.
The modifier the keycap reads is the visitor's, so such a demo comes as three cuts, `<demo>.mp4` reading Ctrl/⌘ (for a visitor whose OS the page cannot tell, as with the site's OS wording), `<demo>.ctrl.mp4` reading Ctrl and `<demo>.cmd.mp4` reading ⌘ (one poster, the default's), and the docs head script swaps the source by OS; a locale without an entry gets uncaptioned videos, so captions are part of translating a locale, followed by regenerating it.
...
A figure of the docs is drawn from data, not kept as a file — the overview (what goes in, ZenCopy, what comes out; named for what it shows, not for the section that happens to use it): [site/src/lib/overview-diagram.ts](site/src/lib/overview-diagram.ts) holds its labels per locale and builds the SVG (five boxes a side — a mail, a chat, a screen, a file, whatever you copy, and the five prompts, the last box of each side saying "anything" under its name; the prompt names come from the app's own messages, or the prompt files' labels where a locale names none, never a second translation; chips are sized to the labels' ink, measured on the build machine's fonts, so no language has to fit a box; a right-to-left locale gets the figure mirrored, inputs on the right; light theme only, like every picture and video of the docs), the endpoint under [site/src/pages/[lang]/diagrams/](site/src/pages/[lang]/diagrams/) rasterizes it to a 2× PNG at build the way og.png is, and the docs embed it through [site/src/components/Diagram.astro](site/src/components/Diagram.astro); a locale gets the figure the moment its labels exist, so translating the labels is part of translating a locale.
...
- The ja docs are the reference the other locales are translated from, so their prose never hard-codes the reader's language: wherever it means _the language this page is in_, it renders [site/src/components/PageLanguage.astro](site/src/components/PageLanguage.astro), whose `form` attribute picks the shape the sentence needs (`name` 日本語, `in` 日本語で, `into` 日本語に) from [src/lib/language-forms.ts](src/lib/language-forms.ts) — a `Record<Locale, …>`, so every locale must define all three; a translation keeps the component and picks the form its own sentence needs.
  ...
  Frontmatter reaches only text-only places — the meta tags, which search engines and link previews read without a script, and the llms.txt files — so a page's `description` writes `{mod}` where the modifier goes (the landing copy's token too) and the docs collection's schema ([site/src/content.config.ts](site/src/content.config.ts)) spells it the neutral `Ctrl/⌘` as the collection loads, before anything reads it — the one spelling in [src/lib/modifier.ts](src/lib/modifier.ts), which the landing page's meta, its prose before the script runs, its animation, and the neutral screenshot and demo cuts all take from there.
...
- **A picture comes before the words for it.** A new screen may be announced by one sentence (what appears), then comes its picture, then the reader's action on it — pressing, typing, choosing, whatever that screen asks — so nobody is told to act on something they have not yet seen; a video or figure likewise shows first, and the sentence saying what it shows follows. Every locale keeps this order.
...
- **The popup resizes through its own handles on Linux and Windows** ([src/components/resize-handles.tsx](src/components/resize-handles.tsx)): the platforms' resize zone for an undecorated window is the window's outer edge, which the floating card's transparent margin puts out of reach, so the card's visible edge carries invisible strips that start a native resize drag (they begin just inside the runtime's zone, never over it). macOS resizes a borderless window natively and the resize drag API is unsupported there, so the handles do not render. The header is the drag handle through its own pointer handler, not `data-tauri-drag-region`: Tauri's script maximizes the window on a double-click, and the popup has to be maximizable — Windows snaps a dragged window to a screen edge only if it has a maximize box (Win+←/→ needs none) — without ever maximizing by accident.
- **On Linux the popup is not a skip-taskbar window** (`expose_popup_to_shell` in [src-tauri/src/windows.rs](src-tauri/src/windows.rs) clears the conf's `skipTaskbar` at startup): GNOME's Tiling Assistant, Ubuntu's default since 23.10 and the handler of Super+←/→ there, refuses to tile a skip-taskbar window, and on GNOME the flag only hides the panel from Alt+Tab, the overview, and the dock. The flag stays for Windows, where it removes nothing but the taskbar button.
- **The Windows installer's finish page starts with "Create desktop shortcut" cleared** ([src-tauri/nsis/hooks.nsh](src-tauri/nsis/hooks.nsh), wired in as `bundle.windows.nsis.installerHooks`): Tauri includes that file at the top of its installer.nsi, before the finish-page defines, so its `MUI_FINISHPAGE_SHOWREADME_NOTCHECKED` reaches the Modern UI readme slot the box is built on. ZenCopy lives in the tray and nothing opens it from the desktop, so installing is Next, Next, Next, Finish with nothing to un-tick, and the guide shows each of those four screens (a non-engineer checks the screen against the picture) while asking for nothing more.
