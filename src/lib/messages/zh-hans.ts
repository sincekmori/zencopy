import type { Messages } from "./types.ts";

export const zhHans: Messages = {
  common: {
    save: "保存",
    saved: "已保存",
    cancel: "取消",
  },
  settings: {
    title: "ZenCopy 设置",
    tagline: (keys) => `按 ${keys}，立即处理你复制的内容——C 要像双击一样快速按两次。`,
    tabGeneral: "通用",
    position: "弹窗位置",
    positionHint: "结果显示在屏幕的哪个角落",
    cornerTopLeft: "左上",
    cornerTopRight: "右上",
    cornerBottomLeft: "左下",
    cornerBottomRight: "右下",
    theme: "主题",
    language: "界面语言",
    textSize: "文字大小",
    textSizeSmall: "小",
    textSizeStandard: "标准",
    textSizeLarge: "大",
    startup: "登录时启动",
    startupHint: "登录系统时自动启动 ZenCopy",
    confirmSend: "发送图片和文件前确认",
    confirmSendHint: "当要把图片等二进制文件发送给提供商时，弹窗会先询问；文本文件无需确认直接执行",
    devMode: "显示模板变量",
    devModeHint: "供自己编写提示词时使用：弹窗会显示每次复制的变量及其内容",
    stats: "使用统计",
    statsHint:
      "每次运行模型都会在本地记录中添加一行，可用于计算费用。所有记录只保存在本设备上，绝不会发送到任何地方；复制的内容也绝不会被记录。",
    statsOpen: "打开文件夹",
    statsReset: "重置统计",
    statsResetConfirm: "删除所有已记录的统计？",
    statsResetDone: "统计已重置。",
    costsEmpty: "还没有任何记录。",
    costsExport: "导出费用汇总",
    costsError: (models) => `无法计算以下模型的费用：${models}。请在此文件中添加 cost 块：`,
    popupCost: "在弹窗中显示本月费用",
    popupCostHint: "在弹窗标题栏实时显示一个小的估算值。使用统计开启时生效。",
    costLimit: "每月费用上限（USD）",
    costLimitHint: "本月估算达到上限后将停止新的运行。留空表示不限；使用统计开启时生效。",
    costsApproxHint:
      "这里的费用只是估算：失败和中断的运行不计入，模型识别也可能有误。准确金额请查看提供商的账单页面。",
    userContext: "关于你",
    userContextHint: "希望 AI 了解的信息——职责、擅长领域、偏好。会附加到每个提示词。",
    userContextPlaceholders: [
      "公司职员。邮件大多是对外的，所以要礼貌些；其他情况结论先行，保持简短。",
      "公司高管。长文只看要点——先给出需要我做决定的内容，数据要附带依据。",
      "大学生。经常写经济学论文——书面语但别太正式，也欢迎对结构提建议。",
      "不用专业术语，用通俗易懂的话解释。最好能打个比方。",
      "工程师。能用 Python 做数据分析，但没做过 Web。技术术语保留原文即可，回答简短。",
    ],
    userContextClear: "清除",
    userContextCleared: "已清除",
    quickTitle: "快捷提示词",
    quickHint: "弹窗中绑定到数字键 1–4 的四个提示词。拖动可重新排序。",
    resetTitle: "重置",
    resetHint: "将 ZenCopy 恢复到刚安装完成的状态——设置、提示词和使用统计都会被删除",
    resetButton: "初始化",
    resetWarning:
      "此操作无法撤销。AI 设置（包括 API 密钥）、规则、自定义提示词和窗口偏好都将被删除，ZenCopy 将回到首次启动时的状态。",
    resetConfirm: "全部删除并初始化",
    optionSystem: "跟随系统",
    optionLight: "浅色",
    optionDark: "深色",
  },
  popup: {
    placeholder: "像双击一样快速复制两次，结果就会显示在这里。",
    devVars: "模板变量",
    noPrompt: "这次捕获不会自动运行提示词 — 在下方选择一个，或按数字键 1–4 运行。",
    rulesDocs: "了解规则机制",
    confirmSend: "要把这次捕获发送给你的 AI 提供商吗？图片和文件可能比文本花费更多。",
    send: "发送",
    dontAskAgain: "不再询问（可在设置中更改）",
    attachmentTooLarge: (mb) => `内容过大，无法发送 — 每次捕获上限为 ${mb} MB。`,
    unsupportedFile: (name) => `“${name}”属于无法发送给模型的文件类型。`,
    fileUnreadable: (name) => `无法读取“${name}”。`,
    switchPrompt: "切换提示词",
    choosePrompt: "选择提示词",
    failed: (reason) => `失败：${reason}`,
    timedOut: "模型没有响应。请检查网络和提供商设置后重试。",
    emptyResult: "模型返回了空响应。",
    costLimitReached: (limit) => `已达到每月费用上限（${limit}）。可在设置中调整。`,
    monthCost: "本月费用",
    stop: "停止",
    retry: "重试",
    followUp: "输入消息…",
    copy: "复制",
    copied: "已复制",
    close: "关闭",
    openSettings: "打开设置",
    updateHint: (version) => `有可用更新 (v${version})`,
  },
  source: {
    inputLabel: "复制的内容",
    richText: "富文本",
    emptyClipboard: "剪贴板是空的。",
    imageAlt: "复制的图片",
    cannotPreview: "（无法预览）",
  },
  markdown: {
    openLink: "要在浏览器中打开此链接吗？",
    open: "打开",
  },
  about: {
    tagline: "复制两次，立即处理。",
    update: (version) => `更新到 v${version} 并重启应用`,
    updateRestart: (version) => `重启应用以更新到 v${version}`,
    updating: "正在更新…",
    checkUpdates: "检查更新",
    checkingUpdates: "正在检查更新…",
    upToDate: "ZenCopy 已是最新版本。",
    updateCheckFailed: "无法检查更新。",
    support: "支持",
    privacy: "隐私",
    terms: "条款",
    logs: "日志",
  },
  welcome: {
    title: "欢迎使用 ZenCopy",
    start: "用这个密钥开始",
    keyLabel: "Gemini API 密钥",
    keyHint: "免费的 Gemini API 密钥是最快的上手方式——获取地址:",
    otherSetup: "用其他方式设置",
  },
  trigger: {
    awaitingLogin:
      "还差一步：GNOME 在登录时加载触发器扩展。请注销并重新登录一次，然后再次启动 ZenCopy。",
    unsupportedSession: "此桌面会话无法监测触发器 — ZenCopy 需要 Wayland 上的 GNOME 或 X11。",
    failed: "触发器无法启动。详情请查看日志。",
    macosPermissions:
      "还差一步：macOS 需要你的许可才能察觉两次复制。请在“系统设置 → 隐私与安全性”中的“输入监控”里允许 ZenCopy，然后退出 ZenCopy 再重新启动。",
  },
  prompts: {
    builtinLabels: {
      "zencopy-translate": "翻译",
      "zencopy-explain": "解释",
      "zencopy-polish": "润色",
    },
    title: "提示词",
    hint: (keys) => `${keys} 能做什么。也可以从弹窗的结果标签切换。`,
    add: "新建提示词",
    export: "导出（.md）",
    import: "导入",
    importHint: "粘贴共享提示词的 Markdown，或选择文件。",
    importFromFile: "选择文件",
    name: "名称",
    instruction: "指令",
    instructionPlaceholder: "例如：用三个要点总结",
    draft: "用 AI 起草",
    draftHint: "随手写下你想要的效果 — 你的模型会把它整理成正式的指令。",
    draftFailed: "起草失败。请检查 AI 设置后重试。",
    advanced: "高级",
    template: "模板",
    templateHint: "复制的内容可通过 {{ text }} 引用（Liquid）。",
    templateDocs: "如何编写模板",
    roleLabel: "角色",
    view: "查看",
    edit: "编辑",
    remove: "删除",
    failed: (reason) => `失败：${reason}`,
    importNotAPrompt: "这不是提示词：应为 `---` YAML frontmatter，后接正文。",
    importNoLabel: "该提示词没有 label。",
    importInvalidId: (id) => `提示词 id“${id}”无效。`,
    importBuiltinId: (id) => `“${id}”是内置提示词的 id——请修改文件中的 id 后重试。`,
    importReservedId: (id) => `“${id}”以“zencopy-”开头——该 id 前缀专用于预装提示词。`,
    importIdExists: (id) => `id 为“${id}”的提示词已存在——请先删除，或修改文件中的 id。`,
    importTooLarge: "文件过大，无法作为提示词导入。",
    labelExists: (label) => `已存在名为“${label}”的提示词——请换一个名称。`,
  },
  rules: {
    title: "规则",
    hint: "每种捕获类型分别运行哪个提示词。",
    kindText: "文本",
    kindImage: "图片",
    kindFiles: "文件",
    none: "不自动执行",
    overridesTitle: "规则",
    overridesHint: "按应用、窗口或长度分流 — 自上而下优先匹配。",
    overridesDocs: "了解规则机制",
    addOverride: "新建规则",
    anyKind: "任意",
    fieldKind: "捕获类型",
    fieldApp: "应用",
    fieldExec: "可执行文件",
    fieldTitle: "窗口标题",
    fieldUrl: "URL",
    fieldFile: "文件名",
    fieldMinChars: "最少字符数",
    fieldMaxChars: "最多字符数",
    rulePrompt: "运行提示词",
    wildcardHint: "条件之间为 AND 关系；* 匹配任意内容。留空的条件会被忽略。",
    needsCondition: "请至少指定一个条件。",
    needsValidBounds: "最少字符数不能大于最多字符数。",
    moveUp: "上移",
    moveDown: "下移",
  },
  ai: {
    title: "AI",
    hint: "提示词所使用的提供商和模型。",
    disclosure:
      "你复制的内容会直接发送给你配置的提供商 — ZenCopy 没有中转服务器，也没有遥测。API 费用由你自行承担，AI 的输出可能有误。",
    provider: "提供商",
    providerCompatible: "OpenAI 兼容",
    baseUrl: "Base URL",
    model: "默认模型",
    apiKey: "API 密钥",
    apiKeyHint: "仅保存在本设备上。",
    revealKeys: "显示 API 密钥",
    freeKeyLink: "获取免费 API 密钥（Google AI Studio）",
    test: "测试",
    testOk: "连接成功",
    testUnreachable: "连接失败。请检查模型名称、API 密钥、Base URL 和网络。",
    advancedHint: "直接编写配置——企业网关、多个提供商并用、按提示词指定模型都在这里。",
    examplesLink: "查看配置示例",
    invalidJson: "JSON 无效。",
    invalidSchema: "JSON 格式正确，但不是有效的配置。详情见日志。",
    saveFailed: "保存失败。详情见日志。",
    invalidConfig: "ai-sdk-catalog.json 无效。请在设置（AI → JSON）中修复。",
    notConfigured: "请先在设置中配置 AI 提供商。",
  },
};
