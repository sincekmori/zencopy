import type { Messages } from "./types.ts";

export const zhHant: Messages = {
  common: {
    save: "儲存",
    saved: "已儲存",
    cancel: "取消",
  },
  settings: {
    title: "ZenCopy 設定",
    tagline: (keys) => `按 ${keys}，立即處理你複製的內容——C 要像連按兩下一樣快速按兩次。`,
    tabGeneral: "一般",
    position: "彈出視窗位置",
    positionHint: "彈出視窗顯示在螢幕的哪個角落",
    cornerTopLeft: "左上",
    cornerTopRight: "右上",
    cornerBottomLeft: "左下",
    cornerBottomRight: "右下",
    theme: "主題",
    language: "介面語言",
    textSize: "文字大小",
    textSizeSmall: "小",
    textSizeStandard: "標準",
    textSizeLarge: "大",
    startup: "登入時啟動",
    startupHint: "登入系統時自動啟動 ZenCopy",
    confirmSend: "傳送圖片與檔案前確認",
    confirmSendHint:
      "要把圖片等二進位檔案傳送給供應商時，彈出視窗會先詢問；文字檔案無需確認直接執行",
    devMode: "顯示範本變數",
    devModeHint: "供自行撰寫提示詞時使用：彈出視窗會顯示每次複製的變數及其內容",
    stats: "使用統計",
    statsHint:
      "每次執行模型都會在本機記錄中新增一行，可用於計算費用。所有記錄只保存在本裝置上，絕不會傳送到任何地方；複製的內容也絕不會被記錄。",
    statsOpen: "開啟資料夾",
    statsReset: "重設統計",
    statsResetConfirm: "刪除所有已記錄的統計？",
    statsResetDone: "統計已重設。",
    statsOffTitle: "要關閉使用統計嗎？",
    statsOffBody:
      "這份紀錄只屬於你：它保存在本裝置上，絕不會被傳送到任何地方，存在的唯一目的是讓你了解自己的花費。關閉後，費用顯示與上限也將無法使用。",
    statsOffConfirm: "關閉",
    costsEmpty: "還沒有任何記錄。",
    costsExport: "匯出費用彙總",
    costsError: (models) => `無法計算以下模型的費用：${models}。請在此檔案中加入 cost 區塊：`,
    popupCost: "在彈出視窗顯示本月費用",
    popupCostHint: "在彈出視窗標題列即時顯示一個小的估算值。使用統計開啟時生效。",
    costLimit: "每月費用上限（USD）",
    costLimitHint: "本月估算達到上限後將停止新的執行。留空表示不限；使用統計開啟時生效。",
    costsApproxHint:
      "這裡的費用只是估算：失敗與中斷的執行不列入計算，模型判定也可能有誤。準確金額請查看供應商的帳單頁面。",
    userContext: "關於你",
    userContextHint: "希望 AI 了解的資訊——職務、擅長領域、偏好。會附加到每個提示詞。",
    userContextPlaceholders: [
      "上班族。郵件多半是與外部往來，請客氣些；其他情況下，結論先行，簡短就好。",
      "高階主管。長篇文件只看重點——需要決定的優先說明；數字要附上依據。",
      "大學生。常寫經濟學報告——書面用語別太生硬,也歡迎提供架構建議。",
      "不用專業術語，請用白話解釋。歡迎使用比喻。",
      "工程師。會用 Python 做資料分析，但無 Web 經驗。技術術語直接保留即可，回答請簡短。",
    ],
    userContextClear: "清除",
    userContextCleared: "已清除",
    quickTitle: "快捷提示詞",
    quickHint: "彈出視窗中綁定到數字鍵 1–4 的四個提示詞。拖曳可重新排序。",
    resetTitle: "重設",
    resetHint: "將 ZenCopy 恢復到剛安裝完成的狀態——設定、提示詞和使用統計都會被刪除",
    resetButton: "初始化",
    resetWarning:
      "此操作無法復原。AI 設定（包含 API 金鑰）、規則、自訂提示詞與視窗偏好都會被刪除，ZenCopy 會回到首次啟動時的狀態。",
    resetConfirm: "全部刪除並初始化",
    optionSystem: "跟隨系統",
    optionLight: "淺色",
    optionDark: "深色",
  },
  popup: {
    placeholder: "像連按兩下一樣快速複製兩次，結果就會顯示在這裡。",
    devVars: "範本變數",
    noPrompt: "這次擷取不會自動執行提示詞 — 請在下方選擇一個，或按數字鍵 1–4 執行。",
    rulesDocs: "了解規則機制",
    confirmSend: "要把這次擷取傳送給你的 AI 供應商嗎？圖片和檔案可能比文字花費更多。",
    send: "傳送",
    dontAskAgain: "不再詢問（可在設定中變更）",
    attachmentTooLarge: (mb) => `內容過大，無法傳送 — 每次擷取上限為 ${mb} MB。`,
    unsupportedFile: (name) => `「${name}」屬於無法傳送給模型的檔案類型。`,
    fileUnreadable: (name) => `無法讀取「${name}」。`,
    switchPrompt: "切換提示詞",
    choosePrompt: "選擇提示詞",
    failed: (reason) => `失敗：${reason}`,
    timedOut: "模型沒有回應。請檢查網路與供應商設定後重試。",
    emptyResult: "模型傳回了空回應。",
    costLimitReached: (limit) => `已達到每月費用上限（${limit}）。可在設定中調整。`,
    monthCost: "本月費用",
    stop: "停止",
    retry: "重試",
    followUp: "輸入訊息…",
    copy: "複製",
    copied: "已複製",
    close: "關閉",
    openSettings: "開啟設定",
    updateHint: (version) => `有可用的更新 (v${version})`,
  },
  source: {
    inputLabel: "複製的內容",
    richText: "帶格式的文字",
    emptyClipboard: "剪貼簿是空的。",
    imageAlt: "複製的圖片",
    cannotPreview: "（無法預覽）",
  },
  markdown: {
    openLink: "要在瀏覽器中開啟此連結嗎？",
    open: "開啟",
  },
  about: {
    tagline: "複製兩次，立即處理。",
    update: (version) => `更新到 v${version} 並重新啟動應用程式`,
    updateRestart: (version) => `重新啟動應用程式以更新到 v${version}`,
    updating: "更新中…",
    checkUpdates: "檢查更新",
    checkingUpdates: "正在檢查更新…",
    upToDate: "ZenCopy 已是最新版本。",
    updateCheckFailed: "無法檢查更新。",
    support: "支援",
    privacy: "隱私權",
    terms: "條款",
    logs: "日誌",
  },
  welcome: {
    title: "歡迎使用 ZenCopy",
    start: "用這把金鑰開始",
    keyLabel: "Gemini API 金鑰",
    keyHint: "免費的 Gemini API 金鑰是最快的上手方式——取得位置:",
    otherSetup: "用其他方式設定",
  },
  trigger: {
    awaitingLogin:
      "還差一步。GNOME 會在登入時載入觸發器擴充功能。請登出並重新登入一次，然後再次啟動 ZenCopy。",
    unsupportedSession: "此桌面工作階段無法監測觸發器 — ZenCopy 需要 Wayland 上的 GNOME 或 X11。",
    failed: "無法啟動觸發器。詳情請查看記錄檔。",
    macosPermissions:
      "還差一步。macOS 需要你的許可才能察覺兩次複製。請在「系統設定 → 隱私權與安全性」的「輸入監控」中允許 ZenCopy，然後將 ZenCopy 結束一次再重新啟動。",
  },
  prompts: {
    builtinLabels: {
      "zencopy-summarize": "摘要",
      "zencopy-translate": "翻譯",
      "zencopy-explain": "解說",
      "zencopy-polish": "潤飾",
    },
    title: "提示詞",
    hint: (keys) => `${keys} 能做什麼。也可以從彈出視窗的結果標籤切換。`,
    add: "新增提示詞",
    export: "匯出（.md）",
    import: "匯入",
    importHint: "貼上共享提示詞的 Markdown,或選擇檔案。",
    importFromFile: "選擇檔案",
    name: "名稱",
    instruction: "指示",
    instructionPlaceholder: "例如：用三個要點摘要",
    draft: "用 AI 草擬",
    draftHint: "隨手寫下你想要的效果 — 你的模型會把它整理成正式的指示。",
    draftFailed: "草擬失敗。請檢查 AI 設定後重試。",
    advanced: "進階",
    template: "範本",
    templateHint: "複製的內容可透過 {{ text }} 引用（Liquid）。",
    templateDocs: "如何撰寫範本",
    roleLabel: "角色",
    view: "檢視",
    edit: "編輯",
    remove: "刪除",
    removeConfirm: (label) => `要刪除提示詞「${label}」嗎？此操作無法復原。`,
    failed: (reason) => `失敗：${reason}`,
    importNotAPrompt: "這不是提示詞：應為 `---` YAML frontmatter，後接正文。",
    importNoLabel: "此提示詞沒有 label。",
    importInvalidId: (id) => `提示詞 id「${id}」無效。`,
    importBuiltinId: (id) => `「${id}」是內建提示詞的 id——請修改檔案中的 id 後重試。`,
    importReservedId: (id) => `「${id}」以「zencopy-」開頭——該 id 前綴專用於預裝提示詞。`,
    importIdExists: (id) => `id 為「${id}」的提示詞已存在——請先刪除，或修改檔案中的 id。`,
    importTooLarge: "檔案過大，無法作為提示詞匯入。",
    labelExists: (label) => `已存在名為「${label}」的提示詞——請改用其他名稱。`,
  },
  rules: {
    title: "規則",
    hint: "複製時自動執行的提示詞。",
    allKinds: "每次複製",
    perKind: "依類型區分",
    inherit: "與每次複製相同",
    kindText: "文字",
    kindRichText: "格式化文字",
    kindImage: "圖片",
    kindFiles: "檔案",
    none: "不自動執行",
    overridesTitle: "規則",
    overridesHint: "依應用程式、視窗或長度分流 — 由上而下優先比對。",
    overridesDocs: "了解規則機制",
    addOverride: "新增規則",
    edit: "編輯",
    anyKind: "任意",
    fieldKind: "擷取類型",
    fieldApp: "應用程式",
    fieldExec: "執行檔",
    fieldTitle: "視窗標題",
    fieldUrl: "URL",
    fieldFile: "檔案名稱",
    fieldMinChars: "最少字元數",
    fieldMaxChars: "最多字元數",
    rulePrompt: "執行提示詞",
    wildcardHint: "條件之間為 AND 關係；* 比對任意內容。留空的條件會被忽略。",
    needsCondition: "請至少指定一個條件。",
    needsValidBounds: "最少字元數不能大於最多字元數。",
    moveUp: "上移",
    moveDown: "下移",
  },
  ai: {
    title: "AI",
    hint: "提示詞所使用的供應商與模型。",
    disclosure:
      "你複製的內容會直接傳送給你設定的供應商 — ZenCopy 沒有中繼伺服器，也沒有遙測。API 費用由你自行負擔，AI 的輸出可能有誤。",
    provider: "供應商",
    providerCompatible: "OpenAI 相容",
    baseUrl: "Base URL",
    model: "預設模型",
    apiKey: "API 金鑰",
    apiKeyHint: "僅儲存在本機。",
    revealKeys: "顯示 API 金鑰",
    freeKeyLink: "取得免費 API 金鑰（Google AI Studio）",
    test: "測試",
    testOk: "連線成功",
    testUnreachable: "連線失敗。請檢查模型名稱、API 金鑰、Base URL 與網路。",
    advancedHint: "直接撰寫組態——適用於企業閘道、多個供應商並用，或依提示詞指定模型等情況。",
    examplesLink: "查看設定範例",
    invalidJson: "JSON 無效。",
    invalidSchema: "JSON 格式正確，但不是有效的設定。詳情請見記錄。",
    saveFailed: "儲存失敗。詳情請見記錄。",
    invalidConfig: "ai-sdk-catalog.json 無效。請在設定（AI → JSON）中修正。",
    notConfigured: "請先在設定中設定 AI 供應商。",
  },
};
